...
def score_node_classification(embeddings, node_labels, p_labeled=0.1, n_repeat=10):
    ...
    with warnings.catch_warnings():
        ...
        for seed in range(n_repeat):
            ...
            split_train, split_test = next(sss.split(embeddings, node_labels))
            lr_cv.fit(embeddings[split_train], node_labels[split_train])
            predicted = lr_cv.predict(embeddings[split_test])
            f1_micro = f1_score(node_labels[split_test], predicted, average='micro')
            f1_macro = f1_score(node_labels[split_test], predicted, average='macro')
            f1.append((f1_micro, f1_macro))
    f1_micro, f1_macro = np.array(f1).mean(0)
